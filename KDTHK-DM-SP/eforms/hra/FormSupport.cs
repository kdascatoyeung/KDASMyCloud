...
namespace KDTHK_DM_SP.eforms.hra
{
    ...
    {
        public FormSupport(string chaseno)
        {
            InitializeComponent();
        ...
        {
            string refno = chaseno.StartsWith("IT-S") ? FormUtil.GetChaseNoByRefNo("support", chaseno) : chaseno;
        ...
        {
            string query = chaseno.StartsWith("IT-S") ? string.Format("select s_answer from TB_FORM_SUPPORT where s_chaseno = '{0}'", chaseno)
